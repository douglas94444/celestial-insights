...
5. **Domínio e Supabase Auth** — **Custom Domains** no Worker (ou rotas + DNS na zona Cloudflare). `APP_PUBLIC_URL` deve coincidir com o URL público final (ex. `https://teu-dominio.com`). No Supabase: **Authentication → URL configuration** com **Site URL** e **Redirect URLs** a incluir `https://teu-dominio.com` (e `www` se usar), não só `*.workers.dev`, para a sessão e OAuth funcionarem no domínio custom. Se o domínio antes apontava para outro host (ex. Lovable), ajustar ou remover registos `A`/`CNAME` antigos. **Dica:** `nslookup teu-dominio` vs `nslookup …workers.dev` — se os IPs/infra forem claramente diferentes, o hostname pode ainda não estar no mesmo Worker onde estão as secrets de pagamento.
...
7. O checkout redireccionado mostra **apenas cartão de crédito/débito e PIX** (boleto, ATM e cartão pré-pago excluídos via `payment_methods.excluded_payment_types` na preferência — `src/lib/mercadopago/client.ts`).
